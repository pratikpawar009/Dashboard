...
shape:
  ...
  cards: "list[OrgSummaryCard { glyph: str, value: str, label: str, sub: str | None }], exactly 5 entries, mockup order: (1) programs_using_ai, (2) total_token_consumption, (3) lines_of_code_generated, (4) releases_using_harness, (5) repos_with_harness_installed_over_total. glyph/label are fixed presentation constants extracted directly from dashboards/CIO Portfolio Dashboard.html's embedded sample-data script (DECISIONS.md D-01): (\"▦\", \"Programs using AI SDLC\"), (\"⬡\", \"Total token consumption\"), (\"</>\", \"Lines of code generated by Harness\"), (\"⤴\", \"Releases using Harness\"), (\"❯\", \"Repos with Harness installed\"). value: cards 1/2/3/4 pass through format_number() (api-conventions) -- card 1's value is format_number(programs_using_ai_count), a bare formatted count, NOT a ratio (DECISIONS.md D-02 flags a mockup-sample discrepancy here, not corrected per Product Gate); card 5 (repos_with_harness_installed_over_total) renders as the literal ratio string \"{repos_with_harness_installed} / {repos_total}\", exempt from format_number() (same treatment as program-detail-api card 4). sub is null on every card (FR-1 decision -- do not invent copy; DECISIONS.md D-02 notes the mockup's own sample data disagrees for card 1, accepted, carried forward)."
  programs_using_ai: "{ count: int, total: int, adoption_percent: float | None } -- count/total are raw ints (needed verbatim for the Adoption Level headline/legend); adoption_percent is None, never 0.0, when total == 0 (BED-02 D-07) -- covers both the missing-row (AC-2) and genuinely-zero-programs cases. Ships as a raw number, never a pre-formatted percent string (FR-1 Decision) -- the frontend composes the headline/subtitle/bar/legend display text from these raw fields (FR-2, DECISIONS.md D-05/D-06), the API ships no color/style/pre-formatted-percent field."
  source: "org_summary_rollup singleton (org_id='org-1'). Row present -> compute_adoption_percent() (api-conventions) derives programs_using_ai; row absent (AC-2, fresh/never-ingested org) -> all-zero fallback, cards render '0'/'0 / 0', programs_using_ai = {count:0, total:0, adoption_percent:None}."
  freshness: "FreshnessAccessor.get_last_successful_run() (freshness-api, BED-04) is called exactly once per request, unconditioned by whether org_summary_rollup returned a row (FR-3) -- always BEFORE the org_summary_rollup query, so a missing system_metadata 'ingestion' row (AC-7) raises HTTPException(500, 'ingestion job may not have run yet') even on an otherwise-valid all-zero payload (the fully-fresh-database case, OVW-01-TC-04). Nothing from this call reaches the response body (AC-6 is backend-only, no freshness timestamp renders anywhere on the page -- resolved research C-1)."
