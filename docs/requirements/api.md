...
shape:
  ...
  items: "list[ProgramTeamRow { member_id: str, member_name: str, role: str, sessions: int, tokens: int, avg_tokens_per_session: int }], field order locked (PGD-05-FR-2, amended by D-06), no additional fields, ordered by tokens DESC. member_id (program_members.user_id) is placed FIRST -- identity before display attributes -- added by D-06 (2026-09-17, docs/features/PGD-05/QUESTIONS.md Q-01) after the row shape was locked without a stable member identifier, which left the per-member usage popup unable to address its own gate/service calls (a display name is never a user id). All three numeric fields are RAW ints -- the renderer owns M/K/B magnitude formatting (mirrors PGD-02, NOT PGD-03's pre-formatted-string precedent). avg_tokens_per_session is tokens/sessions rounded to nearest integer, computed server-side, never float."
  data_source: "Two SELECTs, merged in Python by user_id/user (D-01, ADR-0016): (1) program_members filtered by program_id (identity: user_id, name, role); (2) usage_events filtered by program_id + ts>=range_start, grouped by user (sessions via COUNT(DISTINCT session_id), tokens via SUM(UsageEvent.total) -- usage_events has no column literally named tokens; total is the wire concept's actual column, matching rollup_rebuild.py's existing aggregates) -- backed by NEW index ix_usage_events_program_id_user_ts. A member with zero in-range usage_events is excluded from items (active-in-range contract, decision log (a))."
  ...
  errors: "range outside {7d,30d,90d} -> HTTPException(400, 'invalid_range'), via _range_with_default's Depends() wrapper around validate_range() -- runs before the route body, so this is always the explicit 400, never FastAPI's default 422; 401 if unauthenticated. No 404 -- program_visibility never checks existence, matching PGD-02/04."
  popup_sibling_route: "GET /api/overview/program-detail/{program_id}/team/{member_id}/usage?range= -- a sixth sibling route on the same overview router (app/api/overview.py), NOT part of this contract's response shape. Gated by member_in_program_visibility(current_user, program_id, member_id) (program check AND (self OR cio), D-04) called BEFORE any data fetch -- a denial raises HTTPException(403) with no personal-usage fields in the body (AC-12). On success the handler imports and calls personal-usage-api's own service functions directly -- fetch_card_totals, fetch_daily_token_series, fetch_commands_breakdown (app/services/personal_usage.py) -- NOT an HTTP call to GET /api/personal-usage/{user_id}, with member_id passed as user_id, and returns PersonalUsageResponse (docs/requirements/api.md#personal-usage-api) verbatim with no reshaping (AC-10, D-04). See PGD-05 DATA-DESIGN.md §9 and DECISIONS.md D-04 for the popup route detail."
```
...
consumed_by: [EMD-01]
shape:
  endpoint: "GET /api/overview/program-detail/{program_id}/session-time-series?range=7d|30d|90d&member_id=<optional> (default 30d, via the shared _range_with_default wrapper around validate_range()) -- an eighth sibling on the existing overview router (prefix=/api/overview)"
  response_model: "SessionSeriesResponse @ app/schemas/program_session_series.py -- points: list[SessionPoint], period_total_seconds: int, avg_seconds_per_day: int"
  points: "list[SessionPoint { date: str, session_time_seconds: int }], zero-padded to exactly 7/30/90 entries oldest-to-newest for the resolved range -- a day with no session_series row (org aggregate) or no row for the filtered member gets session_time_seconds: 0, never omitted (PGD-06-FR-4)."
  org_wide_semantics: "CORRECTED from the story's original AC-1 premise (PGD-06/DECISIONS.md D-01): session_series has NO null-member_id rows in practice (_build_session_series always sets member_id=user) -- the unfiltered view SUMs session_time_seconds across every member row for the program, per day. It does NOT filter member_id IS NULL, which would silently match zero rows."
  data_source: "Org-wide: SELECT date_trunc('day', date), SUM(session_time_seconds) FROM session_series WHERE program_id=:pid AND date>=:range_start GROUP BY date_trunc('day', date) -- session_series.date is a DateTime(timezone=True) column, not a date column, so grouping requires the day-bucket truncation (implemented as fetch_program_session_series's day_bucket, mirroring program_detail_token_trend's identical pattern); no member_id filter, no new index (PGD-06/DECISIONS.md D-02, accepted at current data volume). Member-scoped: same query plus AND member_id=:member_id, which DOES match the existing uq_session_series_org_id_program_id_member_id_date constraint's (program_id, member_id, date) prefix. org_id is never read or filtered by this endpoint (D-03) -- consistent with every other PGD sibling service, none of which filter on org_id either."
  derived_fields: "period_total_seconds and avg_seconds_per_day are computed server-side, RAW ints (seconds) -- matching /token-trend's precedent, NOT /releases's pre-formatted-string precedent (PGD-06-FR-2). avg_seconds_per_day = period_total_seconds / <range's fixed day count> (7, 30, or 90), never divided by the count of days that actually have data (PGD-06-FR-5)."
  empty_behavior: "Unknown program_id returns 200 with an all-zero zero-padded series and period_total_seconds: 0 -- NO 404 (PGD-06-FR-3), matching the token-trend/commands/team sibling asymmetry, not releases. A known program_id with zero session_series rows in the window returns the same all-zero shape."
  rbac: "program_visibility() open-aggregate veto gate (AUTH-03) gates the unfiltered/self view, called once with the real program_id -- never filters by current_user.programs, byte-identical across personas. A non-self member_id additionally requires member_in_program_visibility(current_user, program_id, member_id) (self OR cio), called BEFORE any session_series query executes (PGD-06-FR-6, mirroring PGD-05's /team/{member_id}/usage gate-before-query precedent) -- a denial raises HTTPException(403) with no points/period_total_seconds/avg_seconds_per_day fields in the body, logged member_view_denied."
  errors: "range outside {7d,30d,90d} -> HTTPException(400, 'invalid_range') via _range_with_default's Depends() wrapper around validate_range() -- always the explicit 400, never FastAPI's default 422; 401 if unauthenticated. No 404 (see empty_behavior)."
  soft_deleted_members: "A program_roster member with removed_at populated still appears in historical session_series aggregates -- the query filters only on session_series data, never on roster/program_roster status (research Condition C-4, PGD-06-TC-13)."
  observability: "Structured JSON event program_drilldown on a successful fetch ({method, path, program_id, range, status, latency_ms}, matching PGD-03/04/05); member_view_denied is logged by the member_in_program_visibility gate itself on denial, not by this route."
  scope: "Read-only -- never writes session_series (BED-01/BED-03 own that pipeline). No chart or member-filter UI ships with this contract (PGD-06/DECISIONS.md D-05); frontend consumption is via the Next.js proxy apps/web/src/app/api/proxy/program-detail/[program_id]/session-time-series/route.ts (ADR-0008 pattern), rendering deferred to a follow-on story."
```
...
shape:
  ...
  daily_tokens: "DailyTokenSeries { points: list[{date: str, value: str, tokens: int}], period_total: str, avg_per_day: str } — RANGE-scoped (7/30/90 points, zero-padded for a day with no sessions, oldest-to-newest). point.value/period_total/avg_per_day are pre-formatted via format_number() (FR-2's literal field names); point.tokens is a RAW int (ADR-0009 amendment, 2026-09-17, driven by PGD-05's popup chart, FLAGS.md AF-05) — the same computation-input exception commands[].count below already documents, additive alongside value, never a replacement for it. Frontend chart-rendering approach is a separate, still-open decision (docs/requirements/RTM.md § Decisions 2026-09-07) for ARC-01/DEV-01/PMD-01's own consumption, not settled here."
