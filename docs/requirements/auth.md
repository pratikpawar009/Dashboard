...
```yaml
produced_by: [AUTH-01, AUTH-05, AUTH-06]
consumed_by: [AUTH-02, AUTH-03, AUTH-04, AUTH-05, AUTH-06, SHP-01, SHP-02, SHP-03]
shape:
  ...
  program_membership_source: "AUTH-06 (RTM Decisions 2026-09-08): program membership comes from the ROSTER -- `program_members`, populated by ING-10 from each program's committed `.harness/program.yaml` `team[]` -- matched on the session's `email` across the roster's primary address and every entry in its `aliases[]`. Keycloak/OIDC authenticates the user and resolves their persona; it does NOT decide which programs they belong to, and onboarding a program requires no Keycloak group or role. Until AUTH-06 ships, `program_group_parsing` below is still the shipped behaviour."
  program_group_parsing: "SUPERSEDED by program_membership_source above (AUTH-06). Describes what AUTH-01 actually shipped and what still runs today, so it is kept rather than deleted: each string in the groups claim starting with `program_group_prefix` (Settings field, default \"program-\") contributes its remainder as a program-membership entry (e.g. \"program-alpha\" -> \"alpha\"); non-matching groups are dropped; an absent or empty groups claim yields an empty list, never an error. AUTH-06 retires this path along with `PROGRAM_GROUP_PREFIX`, the `groups` entry in `OIDC_SCOPE`, and the Keycloak `groups` client scope + Group Membership mapper in README.md. The `groups` field in `fields` above stays on the session (it is still an IdP claim) -- it simply stops being the membership source."
