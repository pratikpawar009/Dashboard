...
shape:
  ...
  observability: "every resolve() call (cache hit or miss) emits logger.info('persona_mapping_loaded', extra={role, persona, tier, timestamp}) with tier in {tier-1-env, tier-2-yaml, tier-3-postgres}; a tier-3 hit additionally carries tier3_latency_ms. No user_id/email/groups/session context is ever included (PII invariant, unit-tested)."
