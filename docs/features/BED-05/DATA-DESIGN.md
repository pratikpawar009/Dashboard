...
- Org-singleton concurrency (AC-1, D-01): four concurrent `rebuild_org_rollups()` calls (one per AC-1's four distinct programs) each independently compute the *same* current aggregate over `usage_events` and write it via `INSERT ... ON CONFLICT DO UPDATE` (org_summary_rollup) or delete-outside-set + `ON CONFLICT DO UPDATE` (token_series/mau_series) — order-independent per D-01's audit, so all four converge on the identical final row set regardless of which commits first **when the `usage_events` read-set is static**, which is the condition AC-1's test establishes. **Corrected 2026-09-09 (code-review F-1, HIGH):** this does not hold under genuinely concurrent ingest. If `usage_events` is still being written while two `rebuild_org_rollups()` calls are in flight, the call that read earlier can commit later and silently overwrite the fresher aggregate — no exception, no log signal, just a briefly stale row that self-heals on the next full re-derive. Accepted residual risk, not a solved problem; see D-01 § Scope correction and carry-forward `F-1-residual-risk`. No `pg_advisory_xact_lock` is used. `contention_wait_ms` measures time spent waiting on Postgres's own row-level lock during the `ON CONFLICT` UPDATE path (visible only when ≥2 of the four calls target the same row concurrently).
...
- Index support (**corrected 2026-09-09 per D-06 / AF-05 — this bullet's original claim was wrong on both counts**): the four existing `program_id`-prefixed indexes (BED-01) continue to serve program-scoped queries, and T-04's `EXPLAIN ANALYZE` at 160k rows confirms all 7 of them are index-backed. But `ix_usage_events_ts` (D-04) was **never created** — measured, the planner never selects it for any of the 11 rebuild statements, and no expression index matching the month-bucketed `GROUP BY` can legally exist because the 3-arg `date_trunc(text, timestamptz, text)` is `STABLE`, not `IMMUTABLE`. Migration 004 ships `ix_usage_events_program_id_covering` instead, which turns `_build_org_summary` into an Index Only Scan (Heap Fetches 0, ~60ms → ~33ms). **"No query falls back to a sequential scan" is not achievable and is not a defect**: the three org-wide aggregates carry no `WHERE` clause, so at 100% selectivity a sequential scan is the physically correct plan. Do not chase an index for them.
