...
### D-11: `tier3_latency_ms` is emitted on fresh Tier-3 resolutions only — FR-5's allowlist bars PII, not this field · blast:feature · rev:mechanical · adr:—

**Context**: Step 2's gate split on this. `validation-agent` returned FAIL ("the shipped resolver never emits `tier3_latency_ms`"); `code-review-agent` returned PASS WITH WARNINGS on the opposite reading ("the code correctly follows FR-5's literal 4-field schema; the contract doc is stale"). Both identified the same gap and disagreed on which side was wrong.

`REQUIREMENTS.md` genuinely self-conflicts. FR-5 pins the event to `{role, persona, tier, timestamp}` and says "nothing else". But C-4 says "add log field `tier3_latency_ms` to `persona_mapping_loaded` event when tier=3", the § NFR performance line specifies "alert if `tier3_latency_ms` field in `persona_mapping_loaded` events exceeds 200ms p95", the produced contract `docs/requirements/auth.md#persona-resolver` § observability states "a tier-3 hit additionally carries `tier3_latency_ms`", and `tasks.json` T-05's own note reads "persona_mapping_loaded log event per resolution incl. tier3_latency_ms on Tier-3 hits".

**Decision**: implement the field; the code was wrong, not the contract. FR-5 sits under the heading "Log-event field schema … no user context" and its "nothing else" is a **PII allowlist** — it exists to keep `user_id`/`email`/`groups`/`session_id` out of the event. `tier3_latency_ms` is a non-PII operational measure that three other places mandate and that a production alert depends on, so it is additive to FR-5's base schema rather than a violation of it. No further field may be added on this reasoning.

Emitted on a **fresh Tier-3 query only**. A warm cache hit whose stored tier is `tier-3-postgres` deliberately omits it: no query ran, so there is no latency to report, and re-emitting the original measurement on every cached read would dilute the very p95 the NFR alerts on. Presence of the field is therefore the reliable signal that a query actually executed.

`AUTH-02-TC-15`'s exact-allowlist assertion is unchanged — it asserts a Tier-1 event, which still carries exactly the four base fields. Three cases added (`TC-16`): fresh Tier-3 carries the field and nothing else beyond it, Tier-1/Tier-2 omit it, and a warm hit reusing `tier-3-postgres` omits it.

**Root cause of the miss**: T-05's dispatch brief quoted FR-5's four fields and did not carry over the `tier3_latency_ms` requirement from the task's own `notes`. An orchestration error, not an implementer error.
