...
**Tracker Research:** pratikpawar009/Dashboard#245 (https://github.com/pratikpawar009/Dashboard/issues/245)
**Tracker Plan Requirements:** pratikpawar009/Dashboard#246 (https://github.com/pratikpawar009/Dashboard/issues/246)
**Tracker Plan Implementation:** pratikpawar009/Dashboard#251 (https://github.com/pratikpawar009/Dashboard/issues/251)
...
4. Given `POST /auth/dev-bypass` was called with an explicit `programs` list, when the resulting token is used to construct a session, then `session.programs` equals that list exactly, with no `program_roster` query performed — dev-bypass keeps working in an allow-listed environment (AUTH-01) with no roster seed data required. The token carries `programs` as its own top-level JWT claim rather than prefix-encoding it into `groups`, and `get_current_user` takes the bypass branch on `kid == DEV_BYPASS_KID` — the discriminator `_claims_options` already uses (`app/core/auth.py:161`) — never on the mere presence of a `programs` claim, so a Keycloak-minted token carrying one is still resolved from the roster.
...
6. Given this story ships, when `services/api/app/core/config.py`, `services/api/.env.example`, `app/core/auth.py`, and `services/api/app/auth/dev_bypass.py` are inspected, then `Settings.program_group_prefix`/`PROGRAM_GROUP_PREFIX` is removed, `OIDC_SCOPE`'s default no longer includes `groups`, `_parse_programs`'s groups-prefix derivation of `programs` is deleted, and `dev_bypass.py`'s synthetic groups construction (`groups = [f"{settings.program_group_prefix}{program}" ...]`, `dev_bypass.py:98`) is replaced by AC-4's explicit `programs` claim — `session.groups` (the raw IdP claim field on `CurrentUser`) is unchanged and stays populated verbatim when Keycloak sends it; it simply no longer feeds `programs`.
...
- Unit: `services/api/app/core/auth.py` (roster-derived `programs` — match/zero-match/removed-row cases, AC-1..3; dev-bypass passthrough, AC-4; cache/timeout behavior, AC-5), `services/api/app/auth/dev_bypass.py` (explicit `programs` claim replaces the synthetic groups construction, AC-4/AC-6), `services/api/app/core/config.py` / `.env.example` (retired `PROGRAM_GROUP_PREFIX`/`OIDC_SCOPE` groups entry, AC-6).
...
- 2026-09-08 Dev-bypass mechanism resolved — explicit `programs` claim, discriminated by `kid` (AC-4, AC-6): resolves the research-phase clarification raised in `docs/research/AUTH-06.md`. Verified against code: `dev_bypass.py:98` today synthesizes `groups` from `settings.program_group_prefix`, so AC-6's deletion of that setting breaks dev-bypass unless `dev_bypass.py` changes with it — hence its addition to AC-6's file list. The bypass branches on `kid == DEV_BYPASS_KID`, already computed in `get_current_user` and already used by `_claims_options` (`auth.py:161`), because that `kid` only resolves to a signing key under AUTH-01's fail-closed allow-list. Rejected: branching on the presence of a `programs` claim (a Keycloak token carrying one would silently skip the roster) and branching on `email == "dev-bypass@local"` (`payload.email` is caller-supplied, so any caller could assert it).
