...
- 2026-09-08 **SUPERSEDED — program-group prefix retired.** The 2026-08-26 `"program-"` prefix entry above, and AC-5's groups-claim parsing, are superseded by `docs/requirements/RTM.md` § Decisions 2026-09-08: program membership now comes from the `.harness/program.yaml` roster (ING-10) rather than the OIDC `groups` claim, so `PROGRAM_GROUP_PREFIX`, the `groups` entry in `OIDC_SCOPE`, and the Keycloak `groups` client scope + Group Membership mapper all become unnecessary. AUTH-06 removes them. The ACs above are left as written on purpose — they describe what AUTH-01 shipped and what still runs until AUTH-06 lands. Authentication, PKCE, JWKS validation, refresh and dev-bypass are untouched by this.
