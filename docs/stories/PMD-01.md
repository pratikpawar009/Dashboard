...
14. Given the product manager belongs to more than one program, when the program-level panels (program summary, releases, team, program commands) resolve "the program in context," then they use the first entry of the session's roster-derived program list, in the order `program-manifest-api` returns it — assumption (FR-PM-03/FR-AR-03 name "the program in context" without a tie-break rule, and FR-PM-01's ordered component list has no "Switch program" selector, unlike PGD-01's explicit switcher; per Decision log) — until a switcher or default-program setting is specified.
...
- 2026-08-26 Program-in-context tie-break for a multi-program product manager: defaults to the first program in the session's program list — assumption, FR-PM-03/FR-AR-03 name "the program in context" without specifying selection when a persona belongs to more than one program, and FR-PM-01's ordered component list includes no switcher control.
- 2026-09-08 Tie-break re-sourced, not re-decided: the rule above is unchanged ("first in the list"), but the list is no longer `session.groups` in IdP claim order. Per RTM Decisions 2026-09-08, program membership comes from the `.harness/program.yaml` roster (ING-10/AUTH-06), so the ordering this AC tie-breaks on is now the roster's, not the IdP's. Reworded here rather than left stale because PMD-01 is not yet built — it is still `story-validated`, so correcting it now costs nothing, whereas AUTH-01/AUTH-04 shipped against the old model and are superseded by the RTM entry instead of retroactively rewritten.
