...
- 2026-09-08 **SUPERSEDED — scoping source moved to the roster.** The 2026-08-26 entry above ("`session.groups` is the sole source of truth, no separate membership table") is reversed by the user decision recorded in `docs/requirements/RTM.md` § Decisions 2026-09-08: Keycloak is for dashboard login only, and program membership comes from each program's committed `.harness/program.yaml` `team[]` roster (ingested by ING-10, wired into the session by AUTH-06). The acceptance criteria above are **left as written on purpose** — they describe what AUTH-04 actually shipped and what was validated against it, and rewriting them would make the story misrepresent the delivered build. AUTH-06 carries the change; this note is the pointer. What does NOT change: the open-aggregate model, `program_visibility`, and the response shape (ADR-0005) — only where the program list is sourced from.
